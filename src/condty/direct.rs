...
use crate::{ToUint, condty::CondTy, utils};
...
#[track_caller]
pub const fn unwrap_true<C: ToUint, T, F>(tern: CondTy<C, T, F>) -> T {
    ctx!(
        //
...
/// If `C` is zero (even if `T` and `F` are the same type).
#[track_caller]
pub const fn new_true<C: ToUint, T, F>(value: T) -> CondTy<C, T, F> {
    ctx!(
        //
        |c| c.new_true(value),
...
#[track_caller]
pub const fn unwrap_false<C: ToUint, T, F>(tern: CondTy<C, T, F>) -> F {
    ctx!(
        //
...
/// If `C` is nonzero (even if `T` and `F` are the same type).
#[track_caller]
pub const fn new_false<C: ToUint, T, F>(value: F) -> CondTy<C, T, F> {
    ctx!(
        //
...
pub const fn as_ref<C: ToUint, T, F>(tern: &CondTy<C, T, F>) -> CondTy<C, &T, &F> {
...
pub const fn as_mut<C: ToUint, T, F>(tern: &mut CondTy<C, T, F>) -> CondTy<C, &mut T, &mut F> {
...
pub const fn unwrap_trivial<C: ToUint, T>(tern: CondTy<C, T, T>) -> T {
...
///
/// This function is effectively the identity function.
pub const fn new_trivial<C: ToUint, T>(inner: T) -> CondTy<C, T, T> {
    // SAFETY: CondTy<C, T, T> is the same type type as T or T
